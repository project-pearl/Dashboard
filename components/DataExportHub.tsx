...
// Advanced Data Export Hub — multi-format exports, PEARL API docs, standards badges, template downloads, bulk export
...
const TEMPLATES = [
  { name: 'MDE Annual Report', desc: 'Pre-filled MS4 annual compliance report for Maryland Dept. of the Environment', filename: 'PEARL_MDE_Annual_Report_Template.xlsx', icon: '📋' },
  { name: 'EPA 303(d) Submission', desc: 'Impaired waters list submission format per EPA guidance', filename: 'PEARL_EPA_303d_Template.xlsx', icon: '🏛️' },
  { name: 'NPDES DMR Template', desc: 'Discharge Monitoring Report (DMR) template for NPDES permit holders', filename: 'PEARL_NPDES_DMR_Template.xlsx', icon: '📄' },
];
...
export function DataExportHub({ context = 'ms4' }: Props) {
  ...
  const handleFormatExport = (format: ExportFormat) => {
    ...
    setTimeout(() => {
      ...
      a.href = url;
      a.download = `PEARL_Export_${new Date().toISOString().slice(0, 10)}.${ext}`;
  ...
  const handleCopyEndpoint = (path: string) => {
    navigator.clipboard.writeText(`https://api.project-pearl.org${path}`);
  ...
  const handleBulkExport = () => {
    ...
    setTimeout(() => {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `PEARL_Bulk_Export_${bulkDateStart}_to_${bulkDateEnd}.${ext}`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleTemplateDownload = (filename: string) => {
    const blob = new Blob([`Template: ${filename}\nGenerated by PEARL on ${new Date().toISOString()}`], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = filename;
        ...
        {showApiDocs && (
          <div className="p-3 space-y-2 bg-white">
            <div className="flex items-center justify-between mb-1">
              <div className="text-[10px] text-slate-500">Base URL: <code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-mono">https://api.project-pearl.org/api/v1</code></div>
            ...
            <div className="text-[9px] text-slate-400 flex items-center gap-1 pt-1">
              ...
              Full API reference at docs.project-pearl.org/api
            </div>
          </div>
        )}
      </div>
