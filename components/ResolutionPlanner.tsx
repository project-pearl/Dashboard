...
export default function ResolutionPlanner({ scopeContext, userRole, onClose, scenarioContext, dataReliability, dataReady = true }: ResolutionPlannerProps) {
  ...
  }, [scopeContext, userRole, scopeLabel, dataReliability, scenarioContext]);
  ...
  const analysisDescription = (() => {
    switch (scopeContext.scope) {
      ...
      case 'region': {
        const d = scopeContext.data;
        return `Analyze EPA ${d.regionName} (${d.states.join(', ')}): ${d.totalImpaired.toLocaleString()} impaired waterbodies across ${d.states.length} states. Addresses intra-regional coordination, shared watershed issues, and region-specific regulatory actions.`;
      }
      case 'state': {
        const d = scopeContext.data;
        return `Analyze ${d.name} (${d.abbr}): ${d.impaired.toLocaleString()} impaired waterbodies (Cat 5: ${d.cat5.toLocaleString()}, Cat 4a: ${d.cat4a.toLocaleString()}) out of ${d.assessed.toLocaleString()} assessed. Includes state-specific regulatory actions, priority waterbodies, and TMDL development timelines.`;
      }
    }
  })();

  // ── Pre-generation: welcome screen (waits for user action) ──
  if (!plan && !generating) {
    return (
        ...
        <div className="p-6 space-y-5">
          ...
          {!dataReady ? (
            <div className="text-center py-6">
              ...
              <p className="text-sm font-semibold text-gray-700">Loading data sources...</p>
              <p className="text-xs text-gray-400 mt-2">Waiting for ATTAINS and other data sources to finish loading.</p>
            </div>
          ) : (
            <>
              {error && (
  ...
  // ── Generating state (spinner while AI is working) ──
  if (!plan && generating) {
    return (
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-6 py-5 text-white">
        ...
        <div className="p-8">
          <div className="text-center py-6">
            ...
            <p className="text-xs text-gray-400 mt-2">Generating {roleCtx.label}-specific response plan for {scopeLabel}. This takes 30-60 seconds.</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-6 py-4 text-white">
          <div className="flex justify-between items-start">
            ...
            <div className="no-pdf flex gap-2 print:hidden">
              ...
              <button onClick={() => { setPlan(null); setHistory([]); setCustomPrompt(""); }} className="px-3 py-1.5 bg-white/20 hover:bg-white/30 rounded text-sm">New Plan</button>
