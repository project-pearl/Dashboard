...
export default function SiteAssessmentCard({
  ...
  if (bloomSeverity === 'severe' || bloomSeverity === 'significant') {
    treatmentPriorities.push({ rank: 1, driver: `Algal bloom crisis (chlorophyll ${chlVal} ug/L, >${bloomSeverity === 'severe' ? chlSevere : chlSignificant} ${thresholdSourceShort} threshold)`, action: 'Nutrient interception via PIN biofiltration + resin', urgency: 'immediate' });
  ...
  pearlModules.push({ id: 'pearl-core', label: waterType === 'brackish' ? 'PIN — Oyster Biofiltration Unit' : 'PIN — Mussel Biofiltration Unit',
  ...
  if (tmdlStatus === 'needed') {
    regulatory.push({ id: 'tmdl-dev', label: 'TMDL Development', icon: '📋', status: 'warranted', detail: 'Category 5 waterbody requires Total Maximum Daily Load establishment. PIN monitoring data can support load allocation modeling and compliance tracking.', color: 'bg-red-50 border-red-200 text-red-800' });
  }
  ...
  regulatory.push({ id: 'ms4-credit', label: 'MS4 BMP Credit Documentation', icon: '📊', status: 'recommended', detail: 'Document all implemented BMPs and nature-based solutions for MS4 permit compliance credit. PIN real-time data provides verifiable performance metrics.', color: 'bg-indigo-50 border-indigo-200 text-indigo-800' });
  regulatory.push({ id: 'adaptive', label: 'Adaptive Management Plan', icon: '🔄', status: 'recommended', detail: 'Iterative monitoring → assessment → adjustment cycle. Use PIN continuous data to evaluate BMP effectiveness and adjust treatment train over time.', color: 'bg-indigo-50 border-indigo-200 text-indigo-800' });
  ...
  const pearlModel = pearlMods.some(t => t.id === 'pearl-gac' || t.id === 'pearl-ac') ? 'PIN-5X'
    : pearlMods.some(t => t.id === 'pearl-resin-n' || t.id === 'pearl-resin-m') ? 'PIN-3R'
    : pearlMods.some(t => t.id === 'pearl-uv') ? 'PIN-3B'
    : 'PIN-2S';
  ...
  // 1 PIN unit = 50 GPM standard, $200K/unit/year
