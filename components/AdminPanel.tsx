...
import {
  Link2, Users, UserPlus, CheckCircle, XCircle, Clock, Shield,
  Copy, Check, Mail, Building2, MapPin, ChevronDown, ChevronUp,
  AlertTriangle, Trash2, Settings, Send, Globe, GraduationCap,
  RefreshCw, Eye, EyeOff, Lock,
} from 'lucide-react';

// ─── Jurisdiction options (duplicated from AuthForms — extract to shared later) ─
...
export function AdminPanel({ onClose }: AdminPanelProps) {
  const { user, isAdmin, approveUser, rejectUser, deactivateUser, updateUserRole, createInviteLink, listPendingUsers, listAllUsers } = useAuth();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-start justify-center pt-12 px-4 overflow-y-auto">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl border border-slate-200 mb-12">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-gradient-to-br from-cyan-600 to-blue-700">
              <Settings className="h-4.5 w-4.5 text-white" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900">Admin Panel</h2>
              <p className="text-xs text-slate-500">Manage users, invites & approvals</p>
            </div>
          </div>
          <button onClick={onClose} className="px-3 py-1.5 text-sm font-medium text-slate-500 hover:text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors">
            Close
          </button>
        </div>
          ...
          {tab === 'pending' && (
            <div className="space-y-3">
              {loading ? (
                ...
                pending.map(u => (
                  <div key={u.uid} className="p-4 rounded-xl border-2 border-amber-200 bg-amber-50/50 space-y-3">
                    ...
                    {isOperatorRole(u.role) && u.role === 'MS4' && (
                      <div>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={u.role === 'MS4' && !approvalJurisdictions[u.uid]}
              ...
              {loading ? (
                ...
                allUsers
                  .sort((a, b) => {
                    ...
                    const order = { pending: 0, active: 1, rejected: 2, deactivated: 3 };
