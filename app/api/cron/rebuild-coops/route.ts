...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    recordCronRun('rebuild-coops', 'success', Date.now() - startTime);
    ...
    Sentry.captureException(err, { tags: { cron: 'rebuild-coops' } });

    notifySlackCronFailure({ cronName: 'rebuild-coops', error: err.message || 'build failed', duration: Date.now() - startTime });

    recordCronRun('rebuild-coops', 'error', Date.now() - startTime, err.message);
    return NextResponse.json(
