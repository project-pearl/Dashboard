...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    recordCronRun('rebuild-nwis-gw', 'success', Date.now() - startTime);
    ...
    Sentry.captureException(err, { tags: { cron: 'rebuild-nwis-gw' } });

    notifySlackCronFailure({ cronName: 'rebuild-nwis-gw', error: err.message || 'build failed', duration: Date.now() - startTime });

    recordCronRun('rebuild-nwis-gw', 'error', Date.now() - startTime, err.message);
    return NextResponse.json(
