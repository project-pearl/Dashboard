...
export function calculateESGScore(
  ...
  if (pollutantReduction < 80 && removalEfficiencies) {
    ...
    if (lowestParam) {
      ...
      improvementTips.push(
        `${improvement}% more ${lowestParam[0]} removal would raise ESG score by ~${scoreGain} points`
      );
    }
  }
  ...
  if (ejMetrics?.isEJArea && waterQuality < 80) {
    improvementTips.push(
      'Prioritizing improvements in this EJ area enhances ESG standing and grant eligibility'
    );
  }
...
export function generateESGReport(
  ...
  return `ESG WATER IMPACT REPORT
...
Overall ESG Water Impact Score: ${esgScore.overall}/100 (Grade ${esgScore.grade})
Water Risk Level: ${esgScore.waterRiskLevel}

This report provides environmental, social, and governance (ESG) metrics
...
Report generated by Project Pearl ESG Module
`;
}
