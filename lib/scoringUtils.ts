...
export function scoreToGrade(score: number): { letter: string; color: string; bg: string; textColor: string } {
  ...
  switch (l) {
    case 'A':
      return minor === '+'
        ? { letter, color: 'text-green-700',  bg: 'bg-green-100 border-green-300',    textColor: 'text-green-700' }
        : minor === '-'
        ? { letter, color: 'text-green-600',  bg: 'bg-green-50 border-green-200',     textColor: 'text-green-600' }
        : { letter, color: 'text-green-700',  bg: 'bg-green-100 border-green-300',    textColor: 'text-green-700' };
    case 'B':
      return minor === '+'
        ? { letter, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200', textColor: 'text-emerald-700' }
        : minor === '-'
        ? { letter, color: 'text-teal-600',    bg: 'bg-teal-50 border-teal-200',       textColor: 'text-teal-600' }
        : { letter, color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200', textColor: 'text-emerald-600' };
    case 'C':
      return minor === '-'
        ? { letter, color: 'text-yellow-600',  bg: 'bg-yellow-50 border-yellow-200',   textColor: 'text-yellow-600' }
        : { letter, color: 'text-yellow-700',  bg: 'bg-yellow-50 border-yellow-300',   textColor: 'text-yellow-700' };
    case 'D':
      return minor === '+'
        ? { letter, color: 'text-orange-700',  bg: 'bg-orange-50 border-orange-300',   textColor: 'text-orange-700' }
        : minor === '-'
        ? { letter, color: 'text-orange-500',  bg: 'bg-orange-50 border-orange-200',   textColor: 'text-orange-500' }
        : { letter, color: 'text-orange-600',  bg: 'bg-orange-50 border-orange-200',   textColor: 'text-orange-600' };
    case 'F':
      return { letter, color: 'text-red-700',  bg: 'bg-red-50 border-red-300',         textColor: 'text-red-700' };
    default:
      return { letter, color: 'text-slate-500', bg: 'bg-slate-50 border-slate-200',    textColor: 'text-slate-500' };
  }
}
