...
// ─── Station Registry (auto-generated by discover_stations.py) ──────────────
// Contains 12,000+ confirmed monitoring stations across all 51 states
import registryData from './station-registry.json';

const REGISTRY_REGIONS = registryData.regions as Record<string, { lat: number; lng: number; huc8: string; stateCode: string; name: string }>;
const REGISTRY_USGS = registryData.usgsSiteMap as Record<string, string>;
const REGISTRY_WQP = registryData.wqpStationMap as Record<string, { siteId: string; provider: string; name: string }>;
const REGISTRY_COVERAGE = registryData.coverage as Record<string, { hasData: boolean; sources: string[] }>;
...
export function getWaterbodyDataSources(regionId: string): string[] {
  ...
  const regSources = REGISTRY_COVERAGE[regionId]?.sources ?? [];
